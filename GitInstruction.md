...
git log — это команда, которая показывает историю коммитов, но её можно изменять с помощью множества вариантов, таких как git log --oneline для краткого вывода, git log -p для просмотра изменений в каждом коммите и git log --graph для отображения истории в виде дерева. Другие полезные варианты включают фильтрацию по автору (--author), дате (--since, --until), файлу (index.html) и отображение статистики (--stat) или только имен измененных файлов (--name-only). 
Основные и часто используемые варианты
git log: По умолчанию выводит всю историю коммитов в обратном хронологическом порядке, включая хэш, автора, дату и сообщение.
git log --oneline: Показывает каждый коммит в одну строку, что делает историю более краткой и легко читаемой.
git log -p: Отображает полные изменения (diff) для каждого коммита, показывая добавленный и удаленный код.
git log --stat: Показывает статистику изменений, включая количество добавленных и удаленных строк для каждого файла в коммите.
git log --graph: Визуализирует историю в виде графа, показывая ветки и слияния, что особенно полезно для понимания сложной истории проекта.
git log -n <количество>: Ограничивает вывод указанным числом последних коммитов. 
Варианты для фильтрации и поиска
git log --author="Имя": Фильтрует коммиты по имени автора.
git log --since="YYYY-MM-DD" или git log --until="YYYY-MM-DD": Показывает коммиты, сделанные с определенной даты или до определенной даты.
git log index.html: Показывает только те коммиты, которые затрагивали указанный файл (в данном примере index.html).
git log --name-only: Показывает только имена файлов, измененных в каждом коммите, а не сами изменения.
git log -L: Показывает историю изменений определенной функции или строки в коде. 
